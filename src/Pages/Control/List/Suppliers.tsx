...
export default function ControlSuppliersPage(props: any) {
    ...
    const columns: any[] = [
        { title: 'Fornecedor', dataIndex: ['supplier', 'data', 'name'], key: 'supplier' },
        {
    ...
    const label = 'Fornecedores';
    const workerFilter = 'filter_supplier_id';

    const onMapDataToExport = (data: any) => {
        return data.map((service: any) => {
            return {
                'ID': service.data.id,
                'Fornecedor': service?.relationships?.supplier?.data?.name,
